...
class NSGA2Utils(object):
    ...
    def create_initial_population(self,sil_soc,DunnIndex,nsol):
        population = Population()
        for count in range(self.num_of_individuals):
            individual = self.problem.generateIndividual(sil_soc[count],DunnIndex[count],nsol[count])
            self.problem.calculate_objectives(individual)
            population.population.append(individual)
        return population
    
    def create_children(self, nsol,new_di,new_ss):

        children = []
        child=self.problem.generateIndividual(new_ss,new_di,nsol)
