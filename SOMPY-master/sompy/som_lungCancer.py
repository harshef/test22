...
counter=0
actual_label=[]
data = genfromtxt('F:\IITP\PyCharm projects\UPDATE 16 Apr 2017\SOMBatchTest\SOMPY-master\data_set\LungCancer.csv', delimiter=',',dtype=None,usecols=(0))

actual_label=data

print data
Idata = genfromtxt('F:\IITP\PyCharm projects\UPDATE 16 Apr 2017\SOMBatchTest\SOMPY-master\data_set\LungCancer.csv', delimiter=',',usecols=(d for d in range(1,57)))
print type(Idata)
print Idata
normalizer = NormalizatorFactory.build('var')
Idata=normalizer.normalize(Idata)
chromosome =30 #= int(input("Enter the no. of Chromosomes:"))
