...
class SOM(object):
    ...
    def __init__(self,data, neighborhood, normalizer=None, mapsize=None,mask=None, mapshape='planar',lattice='rect',initialization='pca',training='batch',
                 name='sompy'):
        """
        Self Organizing Map
    ...
    def update_codebook_voronoi(self, training_data, bmu, neighborhood):
        ...
        S = P.dot(training_data)   #each neuron has sum of data samples which are matched to that neuron
        ...
        print "nominator  : ", nom
        print "P.sum(axis=1) and its shape : ", P.sum(axis=1), P.sum(axis=1).shape
        nV = P.sum(axis=1).reshape(1, self.codebook.nnodes)   #indicate how many data sample match to a paritcular neuron
        print "nV :  ", nV
        print "neighborhood.T  :  \n", neighborhood.T
        print "nV.dot(neighborhood.T) and its shape  \n : ",nV.dot(neighborhood.T), nV.dot(neighborhood.T).shape
        denom = nV.dot(neighborhood.T).reshape(self.codebook.nnodes, 1)
        print "denominator  : \n", denom
        new_codebook = np.divide(nom, denom)
        print "new codebook :  \n ", new_codebook
        print "new codebokk round up to decimal 6 : " ,np.around(new_codebook, decimals=6)
...
def _chunk_based_bmu_find(input_matrix, codebook, y2):
    ...
    while i0+1 <= dlen:
        ...
        #print "ddata.T  :  ", ddata.T
        #print "codebook shape  : ", codebook.shape
        d = np.dot(codebook, ddata.T)
        #print "dot product of both : ", d
        d *= -2
        #print "dot product of both after multiplying with (-2): ", d
        #print "y2 (einsum of codebook matrix)  :", y2
        #print "reshape y2  :  ", y2.reshape(nnodes, 1)
        d += y2.reshape(nnodes, 1)
        #print "after reshaping y2 adding it to d  :  ", d
        bmu[low:high+1, 0] = np.argmin(d, axis=0)
        #print "bmu[low:high+1, 0]  :  ",bmu[low:high+1, 0]
        bmu[low:high+1, 1] = np.min(d, axis=0)
        #print "bmu[low:high+1, 1]  : ", bmu[low:high+1, 1]
        del ddata
