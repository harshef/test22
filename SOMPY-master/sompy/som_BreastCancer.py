...
import sompy
from Evolutionary.Evoution1 import Mapsize, generate_matingPool,Generate,mapping,calculate_dunn_index,get_agglomerative_centers
...
counter=0
actual_label=[]
data = genfromtxt('F:\IITP\PyCharm projects\UPDATE 16 Apr 2017\SOMBatchTest\SOMPY-master\data_set\BreastCancer.csv', delimiter=',',dtype=None, usecols=(11) ,skip_header=1)
for i in range(len(data)):
    if data[i]== "benign":
        actual_label.insert(i,0)
    elif data[i]== "malignant":
        actual_label.insert(i,1)

print actual_label
Idata = genfromtxt('F:\IITP\PyCharm projects\UPDATE 16 Apr 2017\SOMBatchTest\SOMPY-master\data_set\BreastCancer.csv', delimiter=',',usecols=(2,3,4,5,6,7,8,9,10),skip_header=1)
print Idata
...
n = 2#int(input("Enter the no. of clusters to be formed:"))
Initial_Label=[]
...
# Kcluster = KMeans(n_clusters=n)
# Kcluster.fit(Idata)
# label = Kcluster.predict(Idata)
# #print label
...
for i in range(1,30):
    ...
    if random_value==0:
        ...
        centers=get_agglomerative_centers(label,Idata)
    elif random_value==1:
        print "RV",random_value
        agglomerative = cluster.AgglomerativeClustering(n_clusters=n, linkage='complete', affinity='euclidean')
        idx = agglomerative.fit_predict(Idata)
...
#plt.show()
H= 6 #int(input("Enter the size of neighbourhood mating pool: ")) #Size of mating pool
data=population
final_label=[]
...
while TDS:
    ...
    if not data.size==0:
        ...
        for i in range(len(population)):
            ...
            nsol=Generate(MatingPool, MappedMatrix, H, population,i,flag,CR=1,F=0.5,mutation_prob=0.6,eta=20) #Generate new solution/Children
            print "Nsol",nsol
